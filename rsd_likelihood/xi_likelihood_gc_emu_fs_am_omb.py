...
class PkLikelihood(Likelihood):
	...
	# Load the linear parameters of the theory model theta_a such that
        ...
        for ll, fs_sample_name in enumerate(self.fs_sample_names):
            self.sp_rmax[fs_sample_name] = self.rmax_spline[ll]
            
        # Binning matrix for correlation function, one for each fs sample
        self.binmat = dict((name, None) for name in self.fs_sample_names)
    ...
    def get_best_fit(self):
        try:
            self.xi0_nl  = self.dd - self.Delta
            self.bf_thetas = np.einsum('ij,j', np.linalg.inv(self.Lab), self.Va)
            self.xi0_lin = np.einsum('i,il', self.bf_thetas, self.templates)
            return self.xi0_nl + self.xi0_lin
        except:
    ...
    def loadData(self):
        ...
        for ii, fs_datfn in enumerate(self.fs_datfns):
            ...
            self.rdats[fs_sample_name] = fs_dat[:,0]
            self.xi0dats[fs_sample_name] = fs_dat[:,1]
            self.xi2dats[fs_sample_name] = fs_dat[:,2]
            ...
            yeses = self.rdats[fs_sample_name] > 0
            nos   = self.rdats[fs_sample_name] < 0
        ...
        self.dd = []        
        for fs_sample_name in self.fs_sample_names:
            self.dd = np.concatenate( (self.dd, self.xi0dats[fs_sample_name], self.xi2dats[fs_sample_name]) )
        ...
        for ss, fs_sample_name in enumerate(self.fs_sample_names):
            ...
            rcut = (self.rdats[fs_sample_name] > self.fs_mmaxs[ss])\
                          | (self.rdats[fs_sample_name] < self.fs_kmins[ss])
            
            for i in np.nonzero(rcut)[0]:     # FS Monopole.
                ii = i + startii
                cov[ii, :] = 0
                cov[ :,ii] = 0
                cov[ii,ii] = 1e25
            
            startii += self.rdats[fs_sample_name].size
            
            rcut = (self.rdats[fs_sample_name] > self.fs_qmaxs[ss])\
                       | (self.rdats[fs_sample_name] < self.fs_kmins[ss])
            
            for i in np.nonzero(rcut)[0]:       # FS Quadrupole.
                ii = i + startii
                cov[ii, :] = 0
                cov[ :,ii] = 0
                cov[ii,ii] = 1e25
    ...
    def combine_bias_terms_xiell(self,bvec, xi0table, xi2table, xi4table):
        '''
        Same as function above but for the multipoles.
        
        Returns r, xi0, xi2, xi4, assuming AP parameters from input xi{ell}ktable
        '''
        ...
        xi0 = np.sum(xi0table * bias_monomials,axis=1)
        xi2 = np.sum(xi2table * bias_monomials,axis=1)
        xi4 = np.sum(xi4table * bias_monomials,axis=1)
        
        return xi0, xi2, xi4
    
    def fs_predict(self, fs_sample_name,zfid, thetas=None):
        """Use the PT model to compute xi_ell, given biases etc."""
        
        pp   = self.provider
        zstr = "%.2f" %(zfid)
        taylorPTs = pp.get_result('taylor_xi_ell_mod')
        rout, xi0table, xi2table, xi4table = taylorPTs[zstr]
       ...
       # Instead of calling the linear parameters directly we will now analytically marginalize over them
        ...
        xi0, xi2, xi4 = self.combine_bias_terms_pkell(bvec, xi0table, xi2table, xi4table)
        tt = np.array([rout,xi0, xi2, xi4])
        #np.savetxt('pells_' + self.zstr + '_' + self.sample_name + '.txt',[kv,p0,p2,p4])
        
        # Put a point at k=0 to anchor the low-k part of the Spline.
        # rout,xi0 = np.append([0.,],rout),np.append([0.0,],xi0)
        # xi2 = np.append([0.,],xi2)
        # xi4 = np.append([0.0,],xi4)
        # tt    = np.array([rout,xi0,xi2,xi4]).T
    ...
    def fs_observe(self,tt,fs_sample_name):
        ...
        # Will deal with window function method later  
        rdat = self.rdats[fs_sample_name] 
        if self.binmat[fs_sample_name] is None:
            ...
            self.binmat[fs_sample_name] = np.array(bin_mat)

        tmp0 = np.dot(self.binmat[fs_sample_name], tt[:,1])
        tmp2 = np.dot(self.binmat[fs_sample_name], tt[:,2])
        tmp4 = np.dot(self.binmat[fs_sample_name], tt[:,3])
        convolved_model = np.concatenate((tmp0,tmp0,tmp2,tmp2,tmp4))
        ...
        # maxk = self.sp_rmax[fs_sample_name]
...
#         convolved_model = np.matmul(self.matWs[fs_sample_name], expanded_model )
        ...
        # Save the model:
        self.xiconv[fs_sample_name] = convolved_model
...
class Taylor_pk_theory_zs(Theory):
    ...
    def initialize(self):
        ...
        for zfid, xi_filename in zip(self.zfids, self.xi_filenames):
            zstr = "%.2f"%(zfid)
            taylors_pk = {}
            ...
            x0s = emu['x0']
            rout = emu['rout']
            derivs_xi0 = [np.array(ll) for ll in emu['derivs0']]
            derivs_xi2 = [np.array(ll) for ll in emu['derivs2']]
            derivs_xi4 = [np.array(ll) for ll in emu['derivs4']]
            
            taylors_xi['x0'] = np.array(x0s)
            taylors_xi['rout'] = np.array(rout)
            taylors_xi['derivs_xi0'] = derivs_xi0
            taylors_xi['derivs_xi2'] = derivs_xi2
            taylors_xi['derivs_xi4'] = derivs_xi4
    ...
    def calculate(self, state, want_derived=True, **params_values_dict):
        ...
        for zfid in self.zfids:
            ...
            derivs0 = self.taylors_xi[zstr]['derivs_xi0']
            derivs2 = self.taylors_xi[zstr]['derivs_xi2']
            derivs4 = self.taylors_xi[zstr]['derivs_xi4']
            
            rout = self.taylors_xi[zstr]['rout']
            xi0table = taylor_approximate(cosmopars, x0s, derivs0, order=3)
            xi2table = taylor_approximate(cosmopars, x0s, derivs2, order=3)
            xi4table = taylor_approximate(cosmopars, x0s, derivs4, order=3)
            
            xitables[zstr] = (rout, xi0table, xi2table, xi4table)
